...
//    Firebase Data...
    func fetchFromDB(completion: @escaping (WorkuotModel) -> ()) {
//        you can fetch the current user Data like...
        guard let _ = Auth.auth().currentUser else {
            completion(WorkuotModel(daysValue: [], maxValue: 0,error: "Please Login!"))
            return
        }
        let db = Firestore.firestore().collection("Workout").document("AJqtAJgKN4mFdZPuG3TS")
        db.getDocument { snap, err in
            guard let doc = snap?.data() else{
                completion(WorkuotModel(daysValue: [], maxValue: 0,error: err?.localizedDescription ?? ""))
                return
            }
            ...
            completion(WorkuotModel(daysValue: daysValue, maxValue: Int(maxValue) ?? 0))
        }
    }
}
...
//                    Showing Error
    ...
    @ViewBuilder
    func GraphView(work: WorkuotModel,size : CGSize) -> some View {
//        Graph...
        HStack(spacing:10){
...
//workOut Model..
struct WorkuotModel: Identifiable {
...
//    dont forget to enable keysharing
